...
QImage MainWindow::composeTileImage(const VideoTile* t, const QSize& target)
{
    ...
    if (hasCam && !hasScreen) {
        drawFit(t->lastCam, QRect(QPoint(0,0), target));
        p.end();
        return bg;
    }
    if (!hasCam && hasScreen) {
        drawFit(t->lastScreen, QRect(QPoint(0,0), target));
        p.end();
        return bg;
    }
    ...
    : QMainWindow(parent)
{
    ...
    cbShareQ_->addItem(QStringLiteral("流畅 (848x480 @8fps q50)"));   // 0
    cbShareQ_->addItem(QStringLiteral("平衡 (960x540 @10fps q55)"));  // 1
    cbShareQ_->addItem(QStringLiteral("清晰 (1280x720 @8fps q60)"));  // 2
    cbShareQ_->addItem(QStringLiteral("高清 (1600x900 @8fps q55)"));  // 3
    cbShareQ_->setCurrentIndex(1); // 默认“平衡”
    ...
    annotRow->addWidget(btnAnnotUndo);   // 放在清空前
    annotRow->addWidget(btnAnnotClear_);
    annotRow->addStretch(1);
    root->addLayout(annotRow);
    ...
    connect(btnAnnotUndo, &QToolButton::clicked, this, [this](){
        ...
        if (!m->undoLastByOwner(edUser->text())) {
            txtLog->append(QStringLiteral("[撤销] 没有可撤销的标注"));
            return;
        }
        // 本地刷新
        updateMainFitted();
    ...
    connect(annotCanvas_, &AnnotCanvas::annotateEvent, this, [this](QJsonObject ev){
        ...
        // 本地应用（target 可能是 "__local__"）
        if (auto* m = modelFor(ev.value("target").toString())) {
            m->applyEvent(ev);
        }

        // 网络发送：若 target="__local__"，改为“我的用户名”
...
void MainWindow::configureCamera(QCamera* cam)
{
    ...
    if (desiredRes.isValid() || chosenFmt != QVideoFrame::Format_Invalid) {
        ...
        txtLog->append(QString("申请相机设置: %1x%2, fmt=%3")
                       .arg(vs.resolution().width())
                       .arg(vs.resolution().height())
                       .arg(int(chosenFmt)));
    } else {
        txtLog->append("使用后端默认相机参数");
    }
}

void MainWindow::hookCameraLogs(QCamera* cam)
{
    connect(cam, &QCamera::statusChanged, this, [this](QCamera::Status s){
        txtLog->append(QString("Camera status -> %1").arg(int(s)));
    });
    connect(cam, &QCamera::stateChanged, this, [this](QCamera::State st){
        txtLog->append(QString("Camera state -> %1").arg(int(st)));
    });
#if QT_VERSION < QT_VERSION_CHECK(6,0,0)
    connect(cam, static_cast<void(QCamera::*)(QCamera::Error)>(&QCamera::error),
            this, [this, cam](QCamera::Error e){
        txtLog->append(QString("Camera error(%1): %2").arg(int(e)).arg(cam->errorString()));
    });
#endif
}
...
QImage MainWindow::makeImageFromFrame(const QVideoFrame &frame)
{
    ...
    if (fmt != lastLoggedFormat_) {
        lastLoggedFormat_ = fmt;
        txtLog->append(QString("检测到视频帧像素格式: %1").arg(fmt));
    }
...
void MainWindow::sendImage(const QImage& img)
{
    ...
    if (!writer.write(scaled)) {
        txtLog->append(QString("JPEG 写入失败: %1").arg(writer.errorString()));
        return;
    }
    buffer.close();
...
void MainWindow::applyShareQualityPreset()
{
    ...
    cbShareQ_->setToolTip(QString("%1: %2x%3 @%4fps q=%5")
                          .arg(name).arg(sz.width()).arg(sz.height()).arg(fps).arg(q));
    txtLog->append(QString("共享画质预设 -> %1 %2x%3 @%4 q%5")
                   .arg(name).arg(sz.width()).arg(sz.height()).arg(fps).arg(q));
}
